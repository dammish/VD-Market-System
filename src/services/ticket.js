...
export async function getUserTicketCount(guildId, userId) {
  try {
    ...
    const typedError = ensureTypedServiceError(error, {
      service: 'ticketService', operation: 'getUserTicketCount',
...
export async function createTicket(guild, member, categoryId, reason = 'No reason provided', priority = 'none') {
  try {
    ...
    if (currentTicketCount >= maxTicketsPerUser) {
      return { success: false, error: `You have reached the maximum number of open tickets (${maxTicketsPerUser}). Please close your existing tickets before creating a new one.` };
    }

    let category = categoryId ?
      guild.channels.cache.get(categoryId) :
      guild.channels.cache.find(c => c.type === ChannelType.GuildCategory && c.name.toLowerCase().includes('tickets'));
    ...
    await logTicketEvent({
      client: guild.client, guildId: guild.id,
      event: { type: 'open', ticketId: channel.id, ticketNumber, userId: member.id, executorId: member.id, reason, priority: priority || 'none', metadata: { channelId: channel.id, categoryName: category?.name || 'Default' } }
    });

    return { success: true, channel, ticketData };

  } catch (error) {
    const typedError = ensureTypedServiceError(error, { service: 'ticketService', operation: 'createTicket', message: 'Ticket operation failed: createTicket', userMessage: 'Failed to create ticket. Please try again in a moment.', context: { guildId: guild?.id, userId: member?.id } });
  ...
  try {
    ...
    if (closedCategoryId && channel.parentId !== closedCategoryId) {
      ...
      if (closedCategory?.type === ChannelType.GuildCategory) {
        try { await channel.setParent(closedCategoryId, { lockPermissions: false }); movedToClosedCategory = true; }
        catch (moveError) { logger.warn(`Could not move ticket to closed category: ${moveError.message}`); }
      }
    }

    // Send vouch prompt in ticket channel for purchase tickets
    if (isPurchaseTicket(ticketData)) {
      try {
        const vouchEmbed = new EmbedBuilder()
          .setTitle('Leave a Vouch')
          .setDescription(`<@${ticketData.userId}>, thank you for your purchase!\n\nPlease click the button below to leave a vouch for your experience.`)
          .setColor(0xffd700)
        ...
        await channel.send({ content: `<@${ticketData.userId}>`, embeds: [vouchEmbed], components: [vouchRow] });
      } catch (err) {
    ...
    } catch (permError) { logger.warn(`Could not update user permissions for closed ticket: ${permError.message}`); }

    // Keep buyer able to see the channel so they can vouch
    if (isPurchaseTicket(ticketData)) {
      try {
        ...
        logger.warn(`Could not restore buyer view permission: ${err.message}`);
      }
    }
    ...
    if (ticketMessage) {
      ...
      const updatedEmbed = createEmbed({ title: embed.title || 'Ticket', description: embed.description || 'Ticket discussion', color: '#e74c3c', fields: embed.fields || [], footer: embed.footer });
    ...
    await logTicketEvent({
      ...
      event: { type: 'close', ticketId: channel.id, ticketNumber: ticketData.id, userId: ticketData.userId, executorId: closer.id, reason, metadata: { closedAt: ticketData.closedAt, movedToClosedCategory } }
    });

    return { success: true, ticketData };

  } catch (error) {
    const typedError = ensureTypedServiceError(error, { service: 'ticketService', operation: 'closeTicket', message: 'Ticket operation failed: closeTicket', userMessage: 'Failed to close ticket. Please try again in a moment.', context: { guildId: channel?.guild?.id, channelId: channel?.id, closerId: closer?.id } });
...
export async function claimTicket(channel, claimer) {
  try {
    ...
    const claimStatusMessage = messages.find(m => m.embeds.length > 0 && (m.embeds[0].title === 'Ticket Claimed' || m.embeds[0].title === 'Ticket Unclaimed'));
    ...
    await logTicketEvent({ client: channel.client, guildId: channel.guild.id, event: { type: 'claim', ticketId: channel.id, ticketNumber: ticketData.id, userId: ticketData.userId, executorId: claimer.id, metadata: { claimedAt: ticketData.claimedAt } } });
    return { success: true, ticketData };
  } catch (error) {
...
export async function reopenTicket(channel, reopener) {
  try {
    ...
    const closeStatusMessage = messages.find(m => m.embeds.length > 0 && m.embeds[0].title === 'Ticket Closed' && m.components.length > 0 && m.components[0].components.some(c => c.customId === 'ticket_reopen:'));
...
export async function deleteTicket(channel, deleter) {
  try {
    ...
    const deleteEmbed = createEmbed({ title: 'Ticket Deleted', description: `This ticket will be permanently deleted in ${TICKET_DELETE_DELAY_SECONDS} seconds.`, color: '#e74c3c', footer: { text: `Ticket ID: ${ticketData.id}` } });
    await channel.send({ embeds: [deleteEmbed] });

    await logTicketEvent({ client: channel.client, guildId: channel.guild.id, event: { type: 'delete', ticketId: channel.id, ticketNumber: ticketData.id, userId: ticketData.userId, executorId: deleter.id, metadata: { deletedAt: new Date().toISOString() } } });
          ...
          try {
            ...
            if (guildConfig.ticketTranscriptChannelId) {
              ...
              if (transcriptChannel?.isSendable()) {
                const transcriptEmbed = new EmbedBuilder().setTitle('Ticket Transcript').setDescription(`Transcript for ticket #${ticketData.id}`).setColor('#3498db').addFields({ name: 'Ticket ID', value: `\`${ticketData.id}\``, inline: true }, { name: 'Channel', value: `#${channel.name}`, inline: true }, { name: 'Generated', value: `<t:${Math.floor(Date.now() / 1000)}:F>`, inline: false });
...
export async function unclaimTicket(channel, unclaimer) {
  try {
    ...
    if (ticketData.claimedBy !== unclaimer.id && !unclaimer.permissions.has(PermissionFlagsBits.ManageChannels)) return { success: false, error: 'You can only unclaim your own tickets or need Manage Channels permission.' };
    ...
    await logTicketEvent({ client: channel.client, guildId: channel.guild.id, event: { type: 'unclaim', ticketId: channel.id, ticketNumber: ticketData.id, userId: ticketData.userId, executorId: unclaimer.id, metadata: { previousClaimer } } });
    return { success: true, ticketData };
  } catch (error) {
...
export async function updateTicketPriority(channel, priority, updater) {
  try {
    ...
    if (ticketMessage) {
      const embed = ticketMessage.embeds[0];
      const updatedEmbed = createEmbed({ title: embed.title || 'Ticket', description: embed.description?.split('\n**Priority:**')[0] + `\n**Priority:** ${priorityInfo.emoji} ${priorityInfo.label}`, color: priorityInfo.color, fields: embed.fields || [], footer: embed.footer });
    ...
    await logTicketEvent({ client: channel.client, guildId: channel.guild.id, event: { type: 'priority', ticketId: channel.id, ticketNumber: ticketData.id, userId: ticketData.userId, executorId: updater.id, priority, metadata: { previousPriority: ticketData.priority, updatedAt: ticketData.priorityUpdatedAt } } });
    return { success: true, ticketData };
  } catch (error) {
